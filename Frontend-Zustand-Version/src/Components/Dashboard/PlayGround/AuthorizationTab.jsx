...
const AuthorizationTab = () => {
  const authType = useAppStore(
    (state) => state.activeTabData()?.request?.authType || "No Auth"
  );
  const authData = useAppStore(
    (state) => state.activeTabData()?.request?.authData || {}
  );

  const setAuthType = useAppStore((state) => state.setAuthType);
  const updateAuthData = useAppStore((state) => state.updateAuthData);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (JSON.stringify(localAuthData) !== JSON.stringify(authData)) {
        updateAuthData(localAuthData);
      }
    }, 300);
    return () => clearTimeout(timer);
  }, [localAuthData, authData, updateAuthData]);
