...
export const collectionController = {
    ...
    async fetchChildren(itemId) {
        try {
            const { data } = await api.get(`/collection/items?parentId=${itemId}`);
            return data || [];
        } catch (error) {
    ...
    async createCollection(formData) {
     ...
     async updateCollection(formData) {
        try {
            const { data } = await api.put(`/collection/update`, formData)
            return data;
        } catch (error) {
            console.error('Failed to update collection')
            throw new Error("Faild to update collection", error)
        }
    },
